...
const Religious = () => {
  ...
  const fetchReligious = useCallback(() => {
    try {
      getReligious().then((res) => {
        if (res && res.data) {
          setCard(res.data);
        } else {
        ...
        {card.length > 0 ? (
          ...
          <div className="w-full h-full p-[40px] col-span-3 flex justify-center items-center flex-col">
            <h1 className="flex justify-center items-center gap-2">
              <BiSolidErrorCircle />
              No Religious Blogs Created
            </h1>
            <p className="text-gray-600">
              It looks like there are no religious blogs available at the
